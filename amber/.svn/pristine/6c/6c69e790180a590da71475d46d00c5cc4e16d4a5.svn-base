...
public class SysUserServiceImpl implements SysUserService {
  ...
  public SysUser create(SysUser sysUser) {
    ...
    criteria.or().andAccountEqualTo(sysUser.getAccount().toLowerCase());
    if (sysUserMapper.countByExample(criteria) > 0) {
      throw new UserAccountRepeatException("账号重复");
    }
    // 创建的账号不区分大小写，存入系统一律置为小写
    sysUser.setAccount(sysUser.getAccount().toLowerCase());
  ...
  public SysUser getUserByAccount(String account) {
    SysUserCriteria criteria = new SysUserCriteria();
    // 根据账号获取系统用户对象，账号一律转为小写进行查询
    criteria.createCriteria().andAccountEqualTo(account.toLowerCase());
  ...
  @CacheEvict(value = {"sys-user", "authenticationCache"}, allEntries = true)
  @Override
