...
public class BeanColumnsPlugin extends PluginAdapter {
  ...
  public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
    ...
    if (this.targetPackage != null && this.targetPackage.length() > 0) {
      beanColumnsType = new FullyQualifiedJavaType(this.targetPackage + "." + recordName + BEAN_NAME_SUFFIX);
    } else {
  ...
  protected void addStaticInitBlock(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
    ...
    for (IntrospectedColumn column : columnList) {
      StringBuilder sb =
          new StringBuilder(100).append("columnMap.put(\"").append(column.getJavaProperty()).append("\", \"")
              .append(column.getActualColumnName()).append("\");");
  ...
  protected void addMethod(TopLevelClass topLevelClass) {
    ...
    Parameter parameter = new Parameter(FullyQualifiedJavaType.getStringInstance(), "propertyName");
