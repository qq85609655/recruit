var TagPage = {
    ...
    draw: function() { // 生成标签
        ...
        if(d && d.length > 0) {
            ...
            for(var i = 0; i < d.length; i++ ) {
                str = $("<div class='tag-li'><span class='tag-co' id='tag-co-" + d[i].id + "'>" + d[i].name + " :</span></div>");
                if(!(d[i].labelList && d[i].labelList.length > 0)) {
                    str.append("<span class='font-grey'>无</span>");
                }
                else {
                    for(var j = 0; j < d[i].labelList.length; j++) {
                        temp = $("<a class='tag-a radius' pid='" + d[i].labelList[j].categoryId + "' id='tag-" + d[i].labelList[j].id + "'><span class='tag-a-add'></span>" + d[i].labelList[j].name + "</a>");
    ...
    updateTagsAjax: function(selects,dels,adds) { // 向后台发送更新标签请求
        var tThis = this;
        $.ajax({
            url: basePath + "/resume/updateTags",
            type: "post",
            data: "resumeId=" + tThis.o.resumeId + "&labels=" + selects + "&deletes=" + dels + "&adds=" + adds,
            dataType: "json",
            success: function(msg) {
                if(msg.success) {
