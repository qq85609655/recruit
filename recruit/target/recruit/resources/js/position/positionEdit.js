...
function initDP() {
	var date = new Date();
	setDatePickerValue("effectiveDate",date);
	date.setDate(date.getDate() + 30);
	setDatePickerValue("expectDate",date);
}
...
function eventBind() {
	...
	$('#release').click(function() {
		saveType = "011-002";
		/*if(editType !== "草稿") {
			mini.confirm("该职位" + editType + ",是否重新发布?","",function(action) {
				if(action == "ok") {
	...
	$("#review").click(function() {
		...
		d.departmentRequire.demandType = $("#operateType option:selected").text();
		d.departmentRequire.department = $("#depChoose").val();
	    window.showModalDialog(basePath + "/resources/html/positionPreview.html",d,"dialogWidth=1200px;dialogHeight=650px;resizable=true;");
	});
...
function getPositionId() {
	...
	var _resumeId = _curl.substring(_curl.lastIndexOf("=")+1);
	if(_curl.length == _resumeId.length) {
		return null;
	}
	else {
		return _resumeId;
...
function setPosiData(d) {
	...
	if(d.departmentCode) {
		$("#innerCode").val(d.departmentCode.trim());
	}
	if(d.departmentName) {
		$("#depChoose").val(d.departmentName.trim());
	}
	departmentChoose["#depChoose"] = d.department;
	$("#depDemands").val(d.demands);
	$("#deptManager").val(d.departmentManager);
	setSelectValue("#operateType", d.demandType);
	setSelectValue("#receiveEmail", d.receiveEmail);
	if(editType && !copyFlag) { // 修改
		$("#positionCode").text(d.positionCode);
		setDatePickerValue("effectiveDate", new Date(d.effectiveDate).format("yyyy-mm-dd"));
		setDatePickerValue("expectDate", new Date(d.expectDate).format("yyyy-mm-dd"));
	}
}
...
function getPreviewRequireInforData() {
	return {
		...
		englishLevel : (Number($("#englishLevel").val()) ? ($("#englishLevel option:selected").text() + "以上") : "不限"),
...
function getworkExperienceDec() {
	...
	if(!(a || b)) {
		...
		return $("#workYearLower option:selected").text() + "~" + $("#workYearUpper option:selected").text() + "年";
	}
	else if(a) {
...
function getEducationDec() {
	...
	if(!(a || b)) {
		...
		return $("#fromDegreeIdLower option:selected").text() + "~" + $("#fromDegreeIdUpper option:selected").text();
	}
	else if(a) {
...
function getPositionRequireDesc() {
	positionRequire = "年龄:" + getAgeDec() + "; 工作经验:" + getworkExperienceDec() + "; 学历:" + getEducationDec() + "; 性别:" + $("#gender option:selected").text();
...
function getData() {
	return {
		...
		positionLevel : getSelectValue("#positionLevel"),
		...
		departmentRequire : getDepartmentRequireData(),
		recruitInfor : {
...
function getPreviewData() {
	return {
		...
		positionLevel : $("#positionLevel option:selected").text(),
		...
		positionDescription : editor.html(),
		departmentRequire : getDepartmentRequireViewData(),
		recruitInfor : {
...
function caculateExpiredate() {
	var day = Number($("#validaty").find("option[value='" + $("#validaty").val() + "']").attr("title"));
	var date = mini.get("effectiveDate").getValue();
	var d = new Date(date);
...
$("#editform").validator({
    ...
    fields : {
		...
		"#depDemands"  : {
    		rule : "需求人数:required;integer[*];range[1~999]",
    		msgStyle : "position:absolute;left:100%;"
    	},
    	"#operateType" : {
    		rule : "selectVoid",
    	...
    	"#depChoose" : {
    		rule : "depVoid",
    		msg : "请选择部门",
    		msgStyle : "position:absolute;right:20px;"
    	},
    	"privacy" : {
    		rule : "checked;",
    		msg : "请选择"
