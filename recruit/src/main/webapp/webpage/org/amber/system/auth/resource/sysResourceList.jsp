...
<jsp:include page="/webpage/header.jsp"></jsp:include>
  <div class="co">
    ...
    <div class="co-right">
      <div class="position-path co-right-center">
        ...
        <em class="path-icon">&gt;&gt;</em>
        <span class="path-leaf">资源管理</span>
      </div>
      <div class="co-right-main co-right-center">
        <div class="con-title">资源管理</div>
        <div class="blue radius">
          ...
          <div id="mainTreeGrid" class="mini-treegrid" 
            url="/recruit/auth/resource/sysResource/datagrid" treeColumn="name" idField="id" parentField="parentId"
            resultAsTree="false" allowResize="true" expandOnLoad="false">
  ...
  <script>
      ...
      function create() {
        ...
        if (row) {
          OpenDialog("/recruit/auth/resource/sysResource/create/" + row.id, "新建", tree);
        } else {
          OpenDialog("/recruit/auth/resource/sysResource/create/0", "新建", tree);
        }
      ...
      function modify() {
        var row = tree.getSelected();
        if (row) {
          OpenDialog("/recruit/auth/resource/sysResource/modify/" + row.id, "编辑", tree);
        } else {
          ShowWarnTip("请选择一条记录");
        }
      }
      ...
      function remove() {
        ...
        if (row) {
          mini.confirm(
              ...
              "删除", function(action) {
                if (action == "ok") {
                  SaveData("/recruit/auth/resource/sysResource/remove/" + row.id, "",
                      function(e) {
                        ShowActionTip(e);
                        tree.reload();
                      });
