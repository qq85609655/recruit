...
$(document).ready(function() {
 ...
 function modifyPassowrd() {
    ...
    if (row) {
      mini.open({
        url : "baseUserController/modifyPassword/" + row.id,
        title : "重置密码",
        width : 600,
        height : 360,
        onload : function() {
...
//在画表格的时候为操作添加超链接
  ...
  function setPermissionData(){
	...
	function changePassword() {
      ...
      var row = grid.getSelected();
      if (row) {
        OpenDialog(basePath+"/auth/user/sysUser/changePassword/" + row.id, "重置密码", grid);
      } else {
        ShowWarnTip("请选择一条记录");
      }
    }
	...
	function initAccount(){
      SaveData(basePath+"/baseUserController/initAccount",'');
	}