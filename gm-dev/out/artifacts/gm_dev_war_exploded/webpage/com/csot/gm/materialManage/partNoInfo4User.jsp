...
<body>
      ...
      <div id="handlerMainLayout" class="mini-layout" style="width: 100%; height: 100%;">
        <div title="center" region="center" bodyStyle="overflow:hidden;">
          ...
          <ul id="popupMenu" class="mini-menu" style="display: none;">
            ...
            <li iconCls="icon-download" onclick="exportPartNoByClass" plain="true">选中分类导出信息</li>
            <span class="separator"></span>
            <li iconCls="icon-download" onclick="exportPartNoById" plain="true">选中料号导出信息</li>
            <span class="separator"></span>
  ...
  <script>
      ...
      function exportPartNoByGroup() {
        ...
        var ownerName = ownerNameObj.getValue();
        ownerName = encodeURI(encodeURI(ownerName)); // url传参非asc码必须先转码再传，否则会出现乱码
        var partName = partNameObj.getValue();
        ...
        if (partNo == "" && ownerName == "" && partName == "" && dateBegin == "" && dateEnd == ""
          ...
          var messageid = mini.loading("DownLoading, Please wait ...", "DownLoading");
          $.ajax({
            url : hostPath + "partNo/exportPartNoByGroup?partNo=" + partNo + "&ownerName="
                + ownerName + "&partName=" + partName + "&beginDate=" + dateBegin + "&endDate="
                + dateEnd + "&department=" + department,
            type : "post",
            dataType : 'json', //数据格式为json
            success : function(data) {
              mini.hideMessageBox(messageid);
      ...
      function exportPartNoByProperty() {
        ...
        spec = encodeURI(encodeURI(spec));
        if (brand == "" && spec == "") {
          mini.alert("请输入查询条件");
        } else {
          var messageid = mini.loading("DownLoading, Please wait ...", "DownLoading");
          $.ajax({
            url : hostPath + "partNo/exportPartNoByProperty?brand=" + brand + "&spec=" + spec,
            type : "post",
            dataType : 'json', //数据格式为json
            success : function(data) {
              mini.hideMessageBox(messageid);
