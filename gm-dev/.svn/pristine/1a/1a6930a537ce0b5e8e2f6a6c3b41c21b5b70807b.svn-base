...
<form id="form1" class="fieldset-body" method="post" enctype="multipart/form-data">
  ...
  <%@ include file="/webpage/com/csot/gm/rawMaterial/classAndUserInfo.jsp"%>
      ...
      <legend>
        ...
        <table class="form-table" border="0" cellpadding="13" cellspacing="2">
          <tr>
            ...
            <td class="form-label" style="width: 220px;">最大channel数(CH)：
            <input id="combo2" class="mini-combobox" name="properties['maxChannelId']" onvalidation="onComboValidation"
              style="width: 90px;" valueField="id" url="rawMaterial/getComboData?enumTypeId=${maxChannelEnumTypeId}" required="" allowInput="true"
    ...
    <fieldset class="fieldset" id="fd2" style="width: 650px; font-family: 微软雅黑;">
      <legend>
        <span>料号生成属性（料号规则-电子-适用于3301,3302）</span>
      </legend>
        <table class="form-table" border="0" cellpadding="13" cellspacing="2">
          <tr>
            ...
            <input id="combo4" class="mini-combobox" name="properties['classId']"
  ...
  <script type="text/javascript">
  	...
  	var fn = mini.get("flowNo");
  	  function getFlowNo() {
  	  
      var data = $("#form1").serialize();

      $.ajax({
        url : "rawMaterial/getFlowNo",
        type : "post",
        data : data,
        dataType : 'json', //数据格式为json
        success : function(data) {
		  fn.setValue(data.msg);
        },
          error : function(msg) {
