...
<body>
  ...
  <div class="mini-fit">
    <div id="mainGrid" class="mini-datagrid" style="width: 100%; height: 100%" sizeList="[20,30,50]"
      pageSize="20" url="auth/permission/sysPermission/datagrid?showPager=true" idField="id" allowResize="true">
  ...
  <script>
      ...
      function create() {
        OpenDialog("auth/permission/sysPermission/create", "新建", grid);
      }

      function modify() {
        var row = grid.getSelected();
        if (row) {
          OpenDialog("auth/permission/sysPermission/modify/" + row.id, "编辑", grid);
        } else {
          ShowWarnTip("请选择一条记录");
        }
      }
      ...
      function remove() {
        ...
        if (row) {
          mini.confirm("确定删除记录？", "删除", function(action) {
            if (action == "ok") {
              SaveData("auth/permission/sysPermission/remove/" + row.id, "", function(e) {
                ShowActionTip(e);
                grid.reload();
              });
            }
